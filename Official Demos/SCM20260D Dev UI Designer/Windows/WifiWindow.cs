...
namespace Demos {
    // PORTED TO THE UI DESIGNER. The layout (the two-column form: network-property labels + divider on the
    // right, the SSID/password entry + Connect button on the left, and the status line) is designed in
    // WifiContent.tcui; this window stays an ApplicationWindow (so it keeps its place in the menu
    // navigation), hosts that fragment, and drives its elements with the same WINC15x0 connect logic as before.
    public class WifiWindow : ApplicationWindow {
        ...
        protected override void Active() {
            var content = new WifiContent();
            this.canvas = content;

            this.ipAddressLabel = content.IpAddressLabel;
            this.gatewayLabel = content.GatewayLabel;
            this.subnetMaskLabel = content.SubnetMaskLabel;
            this.dnsLabel1 = content.DnsLabel1;
            this.dnsLabel2 = content.DnsLabel2;
            this.status = content.Status;
            this.ssid = content.Ssid;
            this.password = content.Password;
            this.connectButton = content.ConnectButton;
        ...
        // Adds the shared TopBar chrome (unchanged from the original CreateWindow, which added the TopBar only)
        // to the fragment's canvas.
