...
namespace Demos {
    // PORTED TO THE UI DESIGNER. The layout (status TextFlow + Test button + Next button) is designed in
    // BasicTestContent.tcui; this window stays an ApplicationWindow (so it keeps its place in the menu
    // navigation), hosts that fragment, and drives its elements with the same device test logic as before.
    // Where the original added/removed the Test and Next buttons on the canvas, this version toggles their
    // Visibility instead (they live permanently in the fragment).
    public class BasicTestWindow : ApplicationWindow {
        ...
        private Button nextButton;   // the designed NextButton, grabbed from the fragment
        ...
        protected override void Active() {
            var content = new BasicTestContent();
            this.canvas = content;
            this.textFlow = content.StatusText;
            this.testButton = content.TestButton;
            this.nextButton = content.NextButton;
        ...
        // The status TextFlow and both buttons now live permanently in the BasicTestContent fragment, and the
        // TopBar/BottomBar chrome is added once in Active via AddChrome. There is nothing to rebuild on the
        // canvas here; the visible reset (clearing the text runs, hiding the Test button) is done by the
        // caller and CreateWindow. Kept so the unchanged test-flow code that calls it stays valid.
        private void ClearScreen() {
        }

        private void CreateWindow(bool enableButton) =>
            this.testButton.Visibility = enableButton ? Visibility.Visible : Visibility.Collapsed;
