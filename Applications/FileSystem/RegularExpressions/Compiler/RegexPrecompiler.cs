...
namespace System.Text.RegularExpressions
{
    ...
    public class RegexPrecompiler
    {
        ...
        static public void Main(String[] arg)
        {
            ...
            {
                Debug.WriteLine("Usage: recompile <patternname> <pattern>");
                return;
            }
            ...
            for (int i = 0, end = arg.Length; i < end; i += 2)
            {
                try
                {
                    ...
                    {
                        // End of column?
                        if ((j % numColumns) == 0) Debug.WriteLine("\n        ");

                        // Print char as padded hex number                    
                        String hex = (0).ToString("X4");//.ToHexString();


                        while (hex.Length < 4) hex = "0" + hex;
                        
                        Debug.WriteLine("0x" + hex + ", ");
                    }
                ...
                {
                    Debug.WriteLine("Syntax error in expression \"" + arg[i] + "\": " + e.ToString());
                }
                catch (Exception e)
                {
                    Debug.WriteLine("Unexpected exception: " + e.ToString());
                }
            }
        }
    }
